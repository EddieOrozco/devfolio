...
export default function Navigation() {
  ...
  const handleSmoothScroll = (e, href) => {
    ...
    if (href.startsWith('#')) {
      ...
      if (targetElement) {
        ...
        const navHeight = 20; // Reduced spacing for better alignment
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50">
      {/* Light translucent surface with subtle bottom border */}
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navItems.map((item) => (
              <a
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-slate-700 hover:text-slate-900 hover:bg-slate-100 border border-transparent hover:border-slate-200 transition"
              >
        ...
        {/* Mobile menu */}
        {isMenuOpen && (
          <div className="lg:hidden mt-2 rounded-2xl border border-slate-200 bg-white shadow-md overflow-hidden">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
